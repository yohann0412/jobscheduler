...
class PerformanceMetrics:
    ...
    def record_job_completion(self, job: Dict[str, Any]):
        ...
        with self.lock:
            ...
            # Additional molecular dynamics specific metrics
            if job.get('type') == 'molecular_dynamics':
                simulation_result = job.get('simulation_result', {})
                self.metrics['molecular_dynamics_metrics'] = {
                    'total_particles': len(simulation_result.get('final_particle_states', [])),
                    'final_energy': (
                        simulation_result.get('total_energy', [])[-1]
                        if simulation_result.get('total_energy') else None
                    ),
                    'final_temperature': (
                        simulation_result.get('temperature', [])[-1]
                        if simulation_result.get('temperature') else None
                    )
                }
